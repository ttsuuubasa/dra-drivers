...
func NewDriver(ctx context.Context, config *Config) (*driver, error) {
	...
	config.cacheManager.SetOnUpdate(func() {
		logger := klog.FromContext(ctx)
		logger.Info("Cache state changed, republishing ResourceSlice devices")
		r := resourceslice.DriverResources{
			Pools: map[string]resourceslice.Pool{
				config.flags.nodeName: {
					Slices: []resourceslice.Slice{
						{
		...
		if err := helper.PublishResources(ctx, r); err != nil {
			logger.Error(err, "Failed to republish devices on cache update")
		}
	})
